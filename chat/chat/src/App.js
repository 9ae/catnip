...
class App extends Component {
  ...
  checkBalance(bid) {
    ...
    return window.web3.eth
      ...
      .then(accounts => {
        return Promise.all(
          accounts.map(acc => window.web3.eth.getBalance(acc))
        );
      })
      .then(balances => {
        return balances.map(balance => {
          return balanceIsSufficient(balance, gasPrice, bid);
        });
      })
      .then(sufficients => {
  ...
  handleSubmit(e) {
    e.preventDefault();
    this.checkBalance(this.state.message)
      .then(truth => {
  ...
  render() {
    ...
    if (this.state.load)
      return <div>Please use a browser with MetaMask Installed</div>;
    return (
      <div style={{ textAlign: 'center' }}>
        {this.state.chat_history.map(blob => {
          if (
            this.state.chat_history[this.state.chat_history.length - 1] !==
              blob ||
            ...
            return (
              <div key={blob.message_id}>
                ...
                <span>{blob.sender}</span>
                <br />
                <span>{blob.price}</span>
                <br />
            ...
            return (
              <div key={blob.message_id}>
                ...
                <div>
                  <span
                    onClick={() => {
                      console.log(typeof blob.price, 'bidjfs', blob.price);
                      this.handleAccept(blob.price.toString());
                    }}
                  >
                    ✔
                  </span>
